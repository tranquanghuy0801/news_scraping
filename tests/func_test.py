...
from scripts.preprocess_text import remove_non_ascii, remove_URL, replace_numbers, replace_contractions
...
def test_remove_non_ascii():
    ...
    assert remove_non_ascii(["Huyền", "Tú", "Hoàng"]) == [
        "Huyen", "Tu", "Hoang"]


def test_replace_numbers():
    assert replace_numbers([]) == []
    assert replace_numbers(["sport", "10", "1234"]) == [
        "sport", "ten", "one thousand, two hundred and thirty-four"]
...
def test_sentiment_score():
    assert type(sentiment_score(
        "She is a beautiful girl in my class")) == type(0.55)
