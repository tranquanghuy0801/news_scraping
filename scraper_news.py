...
def scrape_abc(link: str, save_dir: str):
	# initialize dataframe
	dict_pd = {'link': [], 'header': [], 'article': [], 'author': [], 'date': []}
	...
	if len(news) > 0:
		for new in news:
			...
			if len(link_new.split('/')) == 5 and link_new.startswith('/news/'):
				...
				if article_section:
					dict_pd['link'].append(link_new)
					if article_section.find('h1'):
						dict_pd['header'].append(article_section.find('h1').text)
					else:
						dict_pd['header'].append("")
					paragraphs = ' '.join(para.text for para in article_section.find_all('p') if "\n" not in para)
					dict_pd['article'].append(paragraphs)

					if article_section.find('div', class_="byline"):
					...
					if article_section.find('span', class_="timestamp"):
						date = article_section.find('span', class_="timestamp").text
						date = re.findall(r'(?:January|February|March|April|May|June|July|August|September|October|November|December)\s(?:\d|\d{2}),\s\d{4}', date)
						if len(date) > 0:
							dict_pd['date'].append(date[0])
						else:
							dict_pd['date'].append("")
